...
app = FastAPI(title="QICS Python Backend + Frontend", version="1.0.0")
...
@app.get("/api/health")
def health():
    return {"ok": True, "message": "QICS backend running"}
...
def rsp(from_: str, to: str, db: Session = Depends(get_db)):
    ...
    return {
        "summary": {k: res[k] for k in ['days','cum_target','cum_actual','achievement_pct']},
        "rows": res['rows']
    }
...
async def dpc_import(file: UploadFile = File(...), db: Session = Depends(get_db)):
    ...
    for _, row in df.iterrows():
        db.add(DPCRecord(
            vin=row['VIN'], model=row['Model'], date=row['Date'],
